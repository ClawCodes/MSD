 ...
 * Author: Ben Jones and Chris Lawton
 */
public class GrayscaleImage {
    ...
    /**
     * Check that the Image is equivalent to another GrayscaleImage instance.
    ...
    public GrayscaleImage normalized(){
        ...
        for (int i = 0; i < imageData.length; i++) {
            for (int j = 0; j < imageData[0].length; j++) {
                scaledArr[i][j] = imageData[i][j] * scale;
            }
        }

        return new GrayscaleImage(scaledArr);
    }
    ...
    public GrayscaleImage mirrored(){
        double[][] reversedArr = new double[imageData.length][imageData[0].length];
        ...
        return new GrayscaleImage(reversedArr);
    }
    ...
    public GrayscaleImage cropped(int startRow, int startCol, int width, int height) throws IllegalArgumentException{
        ...
        double[][] croppedArr = new double[height][width];
        for (int i = 0; i < height; i++) {
            for (int j = 0; j < width; j++) {
                croppedArr[i][j] = imageData[i + startRow][j + startCol];
            }
        }

        return new GrayscaleImage(croppedArr);
    }
    ...
    public GrayscaleImage squarified(){
        int nRows = imageData.length;
        int nCols = imageData[0].length;
        if (nRows == nCols) {
        ...
        if (nRows > nCols) {
            // height > width
            return cropped(0, 0, nCols, nCols);
        } else {
            // width > height
            return cropped(0, 0, nRows, nRows);
        }
    }
