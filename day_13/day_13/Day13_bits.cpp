 ...
 *
 * Returns:
 *   true if the specified bit is set, false otherwise. // set == 1 , cleared == 0
 *
 * Examples:
...
{
    uint32_t temp = (input << (31-b)) & 0x80000000;
    if (temp == 0x80000000)
        return true;
    else
        return false;
...
int NumBitsSet( uint32_t input )
{
    int setCounter = 0;
    for (int i = 0; i <= 31; i++){
        uint32_t temp = (input << i) & 0x80000000;
        if (temp == 0x80000000)
            setCounter++;
    }
  return setCounter;
...
{
    uint32_t temp = (input >> (8 * b)) & 0x000000FF;
  return temp;
}
...
{
    uint32_t maskPos0 = 0x000000FF;
    uint32_t maskedInput = ~(maskPos0 << (8 * b));
    uint32_t temp = input & maskedInput;
    uint32_t valuePos = (value << (8 * b));
    uint32_t result = temp ^ valuePos;
  return result;
}
